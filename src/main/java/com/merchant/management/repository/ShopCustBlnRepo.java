...
public interface ShopCustBlnRepo extends JpaRepository<ShopCustBalanceDetails, Integer> {
	...
	@Query(value = "SELECT * FROM customer_balance_details_tb WHERE cust_bal_email_id =:custEmail AND cust_bal_owner_name =:ownerEmail", nativeQuery = true)
	List<ShopCustBalanceDetails> getCustBalList(String ownerEmail,String custEmail);
