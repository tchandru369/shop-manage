...
public interface CustTransEntityRepo extends JpaRepository<CustomerTransEntity, Integer>{
	...
	@Query(value = "DELETE FROM cust_trans_entity WHERE trans_cust_ref_id =:custRefId AND trans_own_ref_id =:ownerRefId AND trans_cust_order_id =:orderRefId AND trans_pymt_order_status = 'RAP'", nativeQuery = true)
	int deleteTransEntityForBalPymt(@Param("custRefId") String custRefId,
            @Param("ownerRefId") String ownerRefId,
            @Param("orderRefId") String orderRefId);
